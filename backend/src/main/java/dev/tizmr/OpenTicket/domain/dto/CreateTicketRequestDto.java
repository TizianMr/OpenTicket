...
public record CreateTicketRequestDto(
    @NotBlank(message = ERROR_TITLE_LENGTH) @Length(max = MAX_LENGTH, message = ERROR_TITLE_LENGTH)
        String title,
    @NotBlank(message = ERROR_DESCRIPTION_LENGTH)
        @Length(max = MAX_LENGTH, message = ERROR_DESCRIPTION_LENGTH)
        String description) {
  private static final int MAX_LENGTH = 255;
  private static final String ERROR_TITLE_LENGTH = "Title must be between 1 and 255 characters";
  private static final String ERROR_DESCRIPTION_LENGTH =
